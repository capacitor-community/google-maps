...
class MapPreferences {
    ...
    open func updateFromJSObject(_ preferences: JSObject!) {
        if preferences != nil {
            //  update gestures
            let gesturesObject: JSObject! = preferences["gestures"] as? JSObject;
            self.gestures.updateFromJSObject(object: gesturesObject);
            //  update controls
            let controlsObject: JSObject! = preferences["controls"] as? JSObject;
            self.controls.updateFromJSObject(object: controlsObject);
            //  update appearance
            let appearanceObject: JSObject! = preferences["appearance"] as? JSObject;
